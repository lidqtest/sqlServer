...
public class PAYMENT_NEW_MERGE {
	...
	public static void main(String[] args) {
		...
		for (String latnId : latnIds) {
			FilenameFilter filter = new PaymentNewFilenameFilterImpl(latnId);
			File dir = new File("/data01/fbk/bill/");
			String[] fileNames = dir.list(filter);
			System.out.println(latnId+"地市找到["+fileNames.length+"]个文件");
			if(fileNames.length>0){
				File[] files=dir.listFiles(filter);
				String outFile="int_act_bil_payment_new-date_no_-"+dateNo+"-latn_id_-"+latnId+"-"+sdf.format(new Date())+".txt";
...
class PaymentNewFilenameFilterImpl implements FilenameFilter {

	private String latnId;

	public PaymentNewFilenameFilterImpl(String latnId) {
		this.latnId = latnId;
	}

	@Override
	public boolean accept(File dir, String name) {
		if (name.startsWith("int_act_bil_payment_new_v3-date_no_") && name.endsWith(".txt") && name.contains(latnId)) {
			return true;
		}
		return false;
	}
